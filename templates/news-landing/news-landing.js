...
function getFilterFromURL() {
  ...
  if (filterParam) {
    // Map encoded text back to filter value
    const filterMap = {
  ...
  // Check for year in path (e.g., /news/2015.html)
...
function updateURL(filterValue) {
  ...
  // Map filter values to their display text
  const filterTextMap = {
...
/**
 * Trims description text to specified length
 ...
 * @param {Array} items - All news items
 * @param {string} filterValue - Selected filter value
...
export default async function decorate(block) {
  try {
    ...
    const showItems = async (page) => {
      ...
      const newsBlock = buildBlock('news-cards', visibleItems);
    ...
    block.append(createFilterSection(handleFilterChange), createDivider(),contentSection);
